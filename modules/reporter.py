...
def _clean_location(finding: dict) -> str:
    """
    Return a clean location string for a finding.
    Prefers file > url > empty string.
    Never returns '—' or 'None'.
    """
    loc = finding.get("file") or finding.get("url") or ""
    # Trim long URLs to last 35 chars for table display
    return loc[-35:] if len(loc) > 35 else loc


def _clean_line(finding: dict) -> str:
    """
    Return a clean line number string.
    Returns the line number if present and meaningful,
    otherwise empty string — never '?'.
    """
    line = finding.get("line_hint") or finding.get("line")
    if line and str(line).strip() not in ("", "?", "None", "0"):
        return str(line)
    return ""
...
def print_summary(console, findings, score, score_label, session_id):
    ...
    ai_count    = sum(1 for f in findings
                      if f.get("ai_specific") or f.get("source") == "ollama_ai")
    ...
    t.append(
        ...
        f"OWASP Categories: {owasp_hit}/10  |  "
        f"AI Issues: {ai_count}\n\n"
    )
    ...
    console.print(Panel(
        t,
        title="[bold]WaspSting — Scan Summary[/bold]",
...
def print_findings_table(console, findings):
    ...
    t.add_column("Location", width=30, style="dim")

    for f in sort_findings(findings):
        sev = f.get("severity", "INFO")
        loc = _clean_location(f)
        t.add_row(
            ...
            f.get("title", "")[:45],
            f.get("module", "?"),
            loc or "[dim]response header[/dim]",
        )
    console.print(t)
    console.print()
...
def print_detailed(console, findings):
    ...
    for i, f in enumerate(sort_findings(findings), 1):
        ...
        ai_badge = (
            " [yellow]🤖 AI[/yellow]"
            if f.get("source") == "ollama_ai" or f.get("ai_specific")
            else ""
        )

        console.print(
            f"[{color}]▶ [{i}] {SEVERITY_EMOJI[sev]} {sev} — "
            f"{f.get('title')}{ai_badge}[/{color}]"
        )
        console.print(
            f"   [cyan]OWASP:[/cyan] {f.get('owasp_id', '—')} "
            f"{f.get('owasp_name', f.get('category', ''))}"
        )

        # ── Location line — only show what's actually known ───────────────────
        module   = f.get("module", "?")
        location = _clean_location(f)
        line_no  = _clean_line(f)

        loc_str = location if location else "response"
        line_str = f" (line {line_no})" if line_no else ""
        console.print(
            f"   [cyan]Module:[/cyan] {module}  |  "
            f"[cyan]Location:[/cyan] {loc_str}{line_str}"
        )
        ...
        if f.get("cvss_score") is not None:
            manual = " [dim]✎ manual[/dim]" if f.get("cvss_overridden") else ""
            console.print(
                f"   [cyan]CVSS v3.1:[/cyan] {f['cvss_score']} "
                f"{f.get('cvss_severity','')}  "
                f"[dim]{f.get('cvss_vector','')}[/dim]{manual}"
            )
        ...
        console.print(
            f"   [bold green]Fix:[/bold green] {f.get('fix', 'See OWASP guidelines')}"
        )
        console.print()
...
def print_owasp_map(console, findings):
    console.print(
        Rule("[bold]OWASP Top 10:2025 Coverage[/bold]", style="bright_black")
    )
    console.print()
...
def generate_markdown(results: dict, score: int, score_label: str) -> str:
    ...
    lines = [
        ...
        "This report is generated from authorized security testing only.",
        "",
        "---", "",
        "## Executive Summary", "",
        f"Total findings: **{len(findings)}**  ",
    ...
    # CVSS summary table (if scored)
    extra_md = results.get("extra_md_sections", "")
    if extra_md:
        lines += ["", extra_md]
    ...
    for i, f in enumerate(sort_findings(findings), 1):
        ...
        location = _clean_location(f) or "response"
        line_no  = _clean_line(f)
        line_str = line_no if line_no else "—"

        lines += [
            f"### [{i}] {SEVERITY_EMOJI[f.get('severity','INFO')]} "
            f"{f.get('severity')} — {f.get('title')}{ai_badge}",
            "",
            "| Field | Value |",
            "|-------|-------|",
        ...
        if f.get("cvss_score") is not None:
            ...
            lines.append(
                f"| **CVSS v3.1** | {f['cvss_score']}{manual} "
                f"{f.get('cvss_severity','')} "
                f"`{f.get('cvss_vector','')}` |"
            )

        lines += ["", f"**Description:** {f.get('description','')}", ""]
        ...
        lines += [
            f"**Remediation:** {f.get('fix', 'See OWASP guidelines')}",
            "", "---", "",
        ]
